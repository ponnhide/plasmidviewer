...
def _map_feat(fig, ax, ax2, feats, length, labels=None, labelcolors=None, facecolors=None, edgecolors=None, head_length=None, enlarge=1.0, format=1, bottom=300, fontsize=8, display_label=2, display_axis=True, tick_space="auto"):
    ...
    for i, (feat, label, labelcolor, facecolor, edgecolor) in enumerate(zip(feats, labels, labelcolors, facecolors, edgecolors)):
...
def visualize(record, feature_list=None, fontsize=8, label_list=None, labelcolor=None, facecolor=None, edgecolor=None, display_axis=True, title_visible=True, title=None, tick_interval="auto", format=0, inner_diameter=None, fig=None):
    ...
    #try:
        #feature_list.sort(key=lambda x:x.location.end-x.location.start)
    ...
    for i, feat in enumerate(feature_list):
        if "label" in feat.qualifiers:
            labels.append(feat.qualifiers["label"][0])   
        else:
            labels.append(" ")
        labelcolors.append("#000000")
        facecolors.append(misc_colors[i%len(misc_colors)][0])
        edgecolors.append(misc_colors[i%len(misc_colors)][1])
    ...
    if facecolor is None:
        ...
        if type(facecolor) == str:
            facecolors = [fececolor] * len(facecolors) 

        elif type(facecolor) is list:
            for i in range(len(facecolor)):
                if facecolor[i] is None:
                    pass 
                else:
                    facecolors[i] = facecolor[i] 

        elif type(facecolor) is dict:
            for i in facecolor.keys():
                facecolors[i] = facecolor[i] 
    ...
    if labelcolor is None:
        ...
        if type(labelcolor) == str:
            labelcolors = [labelcolor] * len(labelcolors) 

        elif type(labelcolor) is list:
            for i in range(len(labelcolor)):
                if labelcolor[i] is None:
                    pass 
                else:
                    labelcolors[i] = labelcolor[i] 

        elif type(facecolor) is dict:
            for i in labelcolor.keys():
                labelcolors[i] = labelcolor[i] 
    ...
    if edgecolor is None:
        ...
        if type(facecolor) == str:
            edgecolors = [edgecolor] * len(edgecolors) 

        elif type(edgecolor) is list:
            for i in range(len(edgecolor)):
                if edgecolor[i] is None:
                    pass 
                else:
                    edgecolors[i] = edgecolor[i] 
    ...
    ax, y_list, ty_list, fig_width, ylim, bottom = _map_feat(figure, ax, ax2, feature_list, len(record.seq), enlarge=1.0, labels=labels, labelcolors=labelcolors, facecolors=facecolors, edgecolors=edgecolors, fontsize=fontsize, display_axis=display_axis, tick_space=tick_space, format=format, bottom=bottom) 
    ...
    if title_visible == True:
        ...
        bbox_text   = text.get_window_extent(renderer=renderer)
        bbox_text   = Bbox(coordinate.transform(bbox_text))
        bp_text     = ax2.text(0.5, 0.5-bbox_text.height-0.01, str(len(record.seq)) + " bp", ha="center", va="center", fontsize=fontsize if fontsize >= 8 else 8)
...
if __name__ == "__main__":
    ...
    fig, ax = visualize(record, feature_list=feats, title="lentiCas9-EGFP")
